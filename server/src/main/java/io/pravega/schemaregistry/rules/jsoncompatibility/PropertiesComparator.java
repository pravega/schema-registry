...
public class PropertiesComparator {
    ...
    public BreakingChanges checkProperties(JsonNode toCheck, JsonNode toCheckAgainst) {
        ...
        while(propertyFields.hasNext()) {
            ...
            if(toCheck.get("properties").get(propertyField) == null) {
                ...
                if(!jsonCompatibilityCheckerUtils.hasStaticPropertySet(toCheck) && !jsonCompatibilityCheckerUtils.hasDynamicPropertySet(toCheck)) {
                    ...
                    BreakingChanges breakingChanges = jsonCompatibilityChecker.checkNodeType(
                            toCheck.get("properties").get("additionalProperties"),
                ...
                if(!jsonCompatibilityCheckerUtils.hasStaticPropertySet(toCheckAgainst) && !jsonCompatibilityCheckerUtils.hasDynamicPropertySet(toCheckAgainst)) {
                    // assume that pattern properties always matches
                    // TO DO: add regex check for pattern properties
                    BreakingChanges breakingChanges = jsonCompatibilityChecker.checkNodeType(
                            toCheck.get("properties").get(propertyField),
                            toCheckAgainst.get("properties").get("additionalProperties"));
                    if(breakingChanges!= null)
    ...
    private BreakingChanges additionalProperties(JsonNode toCheck, JsonNode toCheckAgainst) {
        ...
        else if(toCheck.get("aaditionalProperties") != null && toCheckAgainst.get("additinalProperties") != null){
            BreakingChanges additionalPropertiesBreakingChanges = jsonCompatibilityChecker.checkNodeType(toCheck.get("additionalProperties"), toCheckAgainst.get("additionalProperties"));   
